...
public class Table_KhachHang {
    ...
    // tim khach hang theo taikhoan, matkhau
    public List<Model_KhachHang> checkKhachHang(Model_KhachHang model_khachHang){
        String dieukien[] = new String[]{model_khachHang.getTaiKhoan(), model_khachHang.getMatKhau()};
        List<Model_KhachHang> list = new ArrayList<>();
    ...
    // tim khach hang theo id cua bang hoa don
    public List<Model_KhachHang> getKhachHang(Model_DonHang model_donHang){
        String dieukien[] = new String[]{String.valueOf(model_donHang.getMaKH())};
        List<Model_KhachHang> list = new ArrayList<>();

        Cursor c = db.rawQuery("SELECT * FROM KHACHHANG WHERE MAKH = ?", dieukien);
        c.moveToFirst();
        if (c.getCount() > 0){
            Model_KhachHang khoan = new Model_KhachHang();
            int id = c.getInt(0);
